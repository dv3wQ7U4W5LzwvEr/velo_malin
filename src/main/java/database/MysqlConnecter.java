...
public class MysqlConnecter {
  ...
  public void insertStationDonneesDynamiques(List<StationDisponibilites> stations) {
    ...
    for (StationDisponibilites s : stations) {
      ...
      if (!nom.equals("")) {
        ...
        /*
          Comparaison de la date de derniere maj avec celle en base pour n ajouter que les lignes utiles.
          if (listeDerniereDateModifieeParStation.get(idStation) != dateMajJCDecauxSql) alors on execute l'insert
         */
  ...
  private Map<Integer, Timestamp> getToutesLesDatesDernieresModificationsParId() {
    String requete = "SELECT id_station, max(date_MAJ_JCDecaux) FROM velo_malin.stationsdisponibilites group by id_station;";
    ResultSet resultat = executerRequete(requete);
